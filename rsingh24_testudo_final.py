...
if __name__=="__main__":
    image_frame = []
    video_output = cv2.VideoWriter('testudo_proj_new.avi',cv2.VideoWriter_fourcc('M','J','P','G'), 20, (1920,1080))
    ...
    while(cap.isOpened()):
        ...
        if ret == True:
            ...
            for i,  rect_corn in enumerate(rectangles):               
                ...
                AR_TAG =  AR_info[-1]
                if(np.isnan(AR_TAG[1]).any() or type( AR_TAG[2])==type(None) or type( AR_TAG[3])==type(None)):
                    continue
                if( AR_TAG[3]!=7):
                    continue
                test = np.copy(testudo)
                test_new = np.rot90(test, AR_TAG[2]//90,(0,1))
                test_new = cv2.resize(test_new, ( AR_TAG[4], AR_TAG[4]))
                projected_image = Inverse_warping(img, homo, (int(world_homo_frame),int(world_homo_frame)), test_new)
                projected_image = cv2.medianBlur(projected_image, 3)
                video_output.write(projected_image)
                break
